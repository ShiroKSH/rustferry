...
The client trusts one repository, one trusted branch or tag, one generated workflow digest, one
verified worker binary, and one protected GitHub Environment. A build request binds an exact source
commit, source manifest, temporary ref, workflow path, workflow digest, signing references, hashed device identity,
and requested artifacts. Branch names and GitHub run status are routing evidence, not artifact
proof.
...
Submission creates one operation-scoped branch below the configured RustFerry namespace with Git
plumbing; it does not switch the caller's branch or modify the index/worktree. The commit starts from
the exact approved source revision, removes every inherited `.github/workflows/*` entry, then adds
only the approved generated workflow and request envelope. Publication is create-only. Run lookup
binds workflow ID, dispatch commit, branch, and `push` event.
...
Signed setup, doctor, and submission query the exact repository and fail closed unless it is private.
Submission repeats this check immediately before publication, and the signing job also requires the
push event's repository metadata to be private. A public repository remains valid only for unsigned
compilation. These checks cannot make an already-uploaded artifact private if repository visibility
changes later; signed execution therefore requires a dedicated private repository with restricted
membership and visibility-change governance.
