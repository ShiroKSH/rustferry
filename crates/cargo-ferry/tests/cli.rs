...
fn source_bundle_fixture(parent: &std::path::Path) -> std::path::PathBuf {
    ...
    .expect("source fixture lockfile");
    fs::write(project.join("ferry.toml"), "schema_version = 1\n").expect("source fixture config");
    fs::write(project.join("src/main.rs"), "fn main() {}\n").expect("source fixture Rust");
    fs::write(project.join(".env"), "TOKEN=excluded\n").expect("sensitive fixture");
    project
}

fn source_bundle_workspace_fixture(parent: &std::path::Path) -> std::path::PathBuf {
    let workspace = parent.join("workspace");
    for package in ["app", "shared", "unrelated"] {
        fs::create_dir_all(workspace.join(package).join("src"))
            .expect("workspace package directory");
    }
    fs::write(
        workspace.join("Cargo.toml"),
        "[workspace]\nmembers = [\"app\", \"shared\", \"unrelated\"]\nresolver = \"3\"\n",
    )
    ...
    .expect("app manifest");
    fs::write(workspace.join("app/ferry.toml"), "schema_version = 1\n").expect("app config");
    fs::write(workspace.join("app/src/main.rs"), "fn main() {}\n").expect("app source");
    for package in ["shared", "unrelated"] {
        fs::write(
...
fn source_bundle_root_workspace_fixture(parent: &std::path::Path) -> std::path::PathBuf {
    let workspace = parent.join("root-workspace");
    for package in ["shared", "unrelated"] {
        fs::create_dir_all(workspace.join(package).join("src"))
            .expect("root-workspace package directory");
    }
    fs::create_dir_all(workspace.join("src")).expect("root package source directory");
    fs::write(
        workspace.join("Cargo.toml"),
        "[package]\nname = \"root-app\"\nversion = \"0.1.0\"\nedition = \"2024\"\n\n[dependencies]\nshared = { path = \"shared\" }\n\n[workspace]\nmembers = [\"shared\", \"unrelated\"]\nresolver = \"3\"\n",
    )
    ...
    .expect("root-workspace lockfile");
    fs::write(workspace.join("ferry.toml"), "schema_version = 1\n").expect("root-workspace config");
    fs::write(workspace.join("src/main.rs"), "fn main() {}\n").expect("root-workspace source");
    for package in ["shared", "unrelated"] {
        fs::write(
            workspace.join(package).join("Cargo.toml"),
            format!("[package]\nname = \"{package}\"\nversion = \"0.1.0\"\nedition = \"2024\"\n"),
        )
        .expect("root-workspace member manifest");
        fs::write(
            workspace.join(package).join("src/lib.rs"),
            format!("pub fn {package}() {{}}\n"),
        )
        .expect("root-workspace member source");
    }
    workspace
}
...
#[test]
fn source_bundle_cli_creates_verifies_and_never_clobbers() {
    let temporary = tempfile::tempdir().expect("temporary directory");
    let project = source_bundle_fixture(temporary.path());
    let output_directory = temporary.path().join("output");
    fs::create_dir(&output_directory).expect("output directory");
    let archive = output_directory.join("source.zip");
    let descriptor = output_directory.join("source.json");

    let inspection = cargo_bin_cmd!("cargo-ferry")
        .args(["--json", "remote", "bundle", "inspect", "--project-dir"])
        .arg(&project)
        .output()
        .expect("inspect source bundle");
    assert!(
        inspection.status.success(),
        "source inspection failed: stdout={} stderr={}",
        String::from_utf8_lossy(&inspection.stdout),
        String::from_utf8_lossy(&inspection.stderr),
    );
    let inspection: Value =
        serde_json::from_slice(&inspection.stdout).expect("source inspection JSON");
    assert_eq!(inspection["status"], "ok");
    assert_eq!(
        inspection["data"]["manifest"]["entries"]
            .as_array()
            .map(Vec::len),
        Some(4)
    );
    assert_eq!(inspection["data"]["symlinks"], serde_json::json!([]));
    assert_eq!(
        inspection["data"]["excluded_sensitive_paths"],
        serde_json::json!([".env"])
    );

    cargo_bin_cmd!("cargo-ferry")
        .args(["remote", "bundle", "create", "--output"])
        .arg(&archive)
        .arg("--descriptor")
        .arg(&descriptor)
        .arg("--project-dir")
    ...
    let original_archive = fs::read(&archive).expect("created source archive");
    let original_descriptor = fs::read(&descriptor).expect("created source descriptor");

    cargo_bin_cmd!("cargo-ferry")
        .args(["remote", "bundle", "verify", "--archive"])
        .arg(&archive)
        .arg("--descriptor")
        .arg(&descriptor)
        .assert()
        .success()
        .stdout(predicate::str::contains("Source bundle verified"));
    ...
    assert_eq!(
        fs::read(&descriptor).expect("preserved descriptor"),
        original_descriptor
    );

    fs::write(&archive, b"tampered").expect("tamper archive fixture");
    cargo_bin_cmd!("cargo-ferry")
        .args(["remote", "bundle", "verify", "--archive"])
        .arg(&archive)
        .arg("--descriptor")
        ...
        .assert()
        .failure()
        .stderr(predicate::str::contains("failed safely"));
}

#[test]
fn source_bundle_cli_rejects_outputs_inside_the_selected_workspace() {
    let temporary = tempfile::tempdir().expect("temporary directory");
    let project = source_bundle_fixture(temporary.path());
    let archive = project.join("source.zip");
    let descriptor = project.join("source.json");
    ...
    cargo_bin_cmd!("cargo-ferry")
        ...
        .stderr(predicate::str::contains(
            "outside the workspace so bundle creation cannot change its own source snapshot",
        ));
...
#[test]
fn source_bundle_cli_includes_only_the_selected_package_dependency_closure() {
    let temporary = tempfile::tempdir().expect("temporary directory");
    let project = source_bundle_workspace_fixture(temporary.path());
    let output = cargo_bin_cmd!("cargo-ferry")
        .args(["--json", "remote", "bundle", "inspect", "--project-dir"])
        .arg(&project)
        .output()
        .expect("inspect workspace source bundle");
    assert!(
        output.status.success(),
        "stdout={} stderr={}",
        String::from_utf8_lossy(&output.stdout),
    ...
    let document: Value = serde_json::from_slice(&output.stdout).expect("inspection JSON");
    let paths = document["data"]["manifest"]["entries"]
        .as_array()
    ...
    assert!(paths.contains(&"shared/src/lib.rs"));
    assert!(!paths.iter().any(|path| path.starts_with("unrelated/")));
}

#[test]
fn source_bundle_cli_excludes_unrelated_members_when_the_project_is_the_workspace_root() {
    let temporary = tempfile::tempdir().expect("temporary directory");
    let project = source_bundle_root_workspace_fixture(temporary.path());
    let output = cargo_bin_cmd!("cargo-ferry")
        .args(["--json", "remote", "bundle", "inspect", "--project-dir"])
        .arg(&project)
        .output()
        .expect("inspect root workspace source bundle");
    assert!(
        output.status.success(),
        "stdout={} stderr={}",
        String::from_utf8_lossy(&output.stdout),
    ...
    assert!(!paths.iter().any(|path| path.starts_with("unrelated/")));
    assert_eq!(
        document["data"]["path_dependencies"],
        serde_json::json!(["shared"])
    );
}

#[test]
fn source_bundle_cli_accepts_explicit_portable_executable_metadata() {
    let temporary = tempfile::tempdir().expect("temporary directory");
    let project = source_bundle_fixture(temporary.path());
    let output = cargo_bin_cmd!("cargo-ferry")
        .args([
        ...
        .arg(&project)
        .output()
        .expect("inspect executable source metadata");
    assert!(
        output.status.success(),
        "stdout={} stderr={}",
        String::from_utf8_lossy(&output.stdout),
    ...
    let document: Value = serde_json::from_slice(&output.stdout).expect("inspection JSON");
    let entry = document["data"]["manifest"]["entries"]
        .as_array()
        .expect("manifest entries")
        .iter()
        .find(|entry| entry["path"] == "src/main.rs")
        .expect("Rust source entry");
